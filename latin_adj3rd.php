...
$adjs = array("acer","omnis","ingens","optans");
...
// we want data read across, but the data file was read top-down, so let's remap:
$fixed = array();
$index = 0;
for ($section = 0; $section < sizeof($adjs); $section++) {
	for ($x = 0; $x < 5; $x++) {
		for ($i = 0; $i < 6; $i++) {
			$fixed[$index] = $answers[30 * $section + 5 * $i + $x];
			$index++;	
		}
	}
}
...
<tr class="column-heading-number">
  <td></td>
  <td class="column-heading-1" colspan="3">singular</td>
