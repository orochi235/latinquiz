...
/* script supports the following arguments via GET:
 *
 * verb			the first principal form of the verb to scrape from verbix. required.
 * nocache		If set to any value of length > 0, prevents the script from saving its output to the latin_cache directory. optional.
 */

if (!array_key_exists("nocache", $_GET) || strlen($_GET["nocache"] == 0))  {	
	ob_start();
}
...
<title>Verb Quiz: <?php echo $_GET["verb"]; ?></title>
<style type="text/css">
...
$forms = scrape($_GET["verb"]);

if ($forms == false) { // if we get false back, it means the verb doesn't exist.
	echo "Verb <b>" . $_GET["verb"] . "</b> not found in Verbix database.";
	ob_flush();
	die();
}
...
// LEGACY CODE FOR READING ANSWERS FROM FILE INSTEAD OF SCRAPER:
/*
...
function writeField($c, $index="") {
	...
	$style = "";
	if ($answers[$c] == "") $style = "opacity: 0	;"; // hide unused forms

	$tabindex = strlen($index) > 0 ? "tabindex=\"$index\" " : "";
	?>
<input type="text" class="entry" name="<?php echo $c; ?>" id="<?php echo $c; ?>" value="<?php //echo $answers[$c]; ?>" style="<?php echo $style; ?>" <?php echo $tabindex; ?> onchange='//update(this);' onblur='//update(this);' onfocus='markActive(this);' onclick='//markActive(this);' />
<?php 
...
<table name="main">
    <tr class="buttons">
        ...
        <td colspan="5" class="buttons" style="text-align: right;">
            <button onclick="fillAnswers();" tabindex="-1">Show Answers</button>
    		<button onclick="clearAll();" tabindex="-1">Clear All</button>
        </td>
    </tr>
  <?php
		...
		for ($col = 0; $col < sizeof($columns) + 1; $col++) {
			...
			if ($col == 0) {
				...
				if ($section % 3 != 2 || (($col - 1) / 2) % 2 != 1) { // don't write fields for subjunctive future tenses
					...
					if ($answers[$fieldName] == "" || !array_key_exists($fieldName, $answers)) $tabindex = -1;
...
if (!array_key_exists("nocache", $_GET) || strlen($_GET["nocache"] == 0))  {	
	...
	ob_end_flush();
	$fp = fopen("./latin_cache/" . $_GET["verb"] . ".html","w");
	fwrite($fp,$page);
	fclose($fp);
}
